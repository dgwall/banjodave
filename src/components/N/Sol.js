import React from "react";
import "./N.css";

function Sol() {
  return (
    <section>
      <header>
        <img src="../img/orb_invert.gif" alt="Under Construction" />{" "}
        Sol+144*10mm
      </header>
      <div className="pack-overview">
        <p>
          Get ready for the launch of <b>Sol+144*10mm</b>, an extensive map pack
          for the platforming game, N v1.4. This upcoming release promises a
          broad spectrum of maps designed to engage players of all skill levels.
          Featuring: 50 original maps, 50 ultra-challenging remix versions, and
          an additional 144 simpler speedmaps.
        </p>

        <p>
          The <b>Sol</b> map pack will feature maps with difficulty levels that
          range from very easy to hard, ensuring a suitable challenge for
          everyone from beginners to experienced players. Each map will also
          house three hidden trapdoor switches, serving as bonus objectives for
          those players who love to explore and conquer.
        </p>

        <p>
          Meanwhile, the <b>s*l</b> maps, remix versions of the original sol
          maps, will deliver unique mechanical changes and ramped-up challenges,
          introducing a fresh perspective to the game's physics-based
          platforming.
        </p>

        <p>
          Lastly, the <b>144*10mm</b> project maps, crafted under the ambitious
          undertaking of creating six maps an hour for 24 separate sessions,
          promise simpler layouts. These maps will be perfect for those seeking
          quick playthroughs or a less intense challenge.
        </p>
      </div>
    </section>
  );
}

export default Sol;
